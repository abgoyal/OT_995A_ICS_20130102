...
static int check_and_run_traps(int sig)
{
	...
	if (sig)
		goto jump_in;
	while (1) {
		//sig = sigtimedwait(&G.blocked_set, NULL, &zero_timespec); ajayet : interface not found
        sig = __rt_sigtimedwait(&G.blocked_set, NULL, &zero_timespec, sizeof(G.blocked_set));
		if (sig <= 0)
			break;
 jump_in:
		last_sig = sig;
...
static int FAST_FUNC builtin_wait(char **argv)
{
	...
	if (argv[0] == NULL) {
		...
		while (1) {
			...
			sig = sigwait(&G.blocked_set, NULL); // ajayet replaced sigwaitinfo()
			if (sig > 0) {
